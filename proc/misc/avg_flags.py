...
def expand_flags(avg_flags, N):
    npol, nchan, nifr, nspw = avg_flags.shape
    tmp_flags = np.reshape(avg_flags, (npol, nchan, nifr, 1, nspw))
    out_flags = np.repeat(tmp_flags, N, axis=3)
    out_flags = np.reshape(out_flags, (npol, nchan, nifr, N * nspw))
    return out_flags
...
#ms_name  = '57519_part1.ms'
ms_name  = sys.argv[-1].strip()
dat_file = '%s/%s' %(dat_dir, ms_name)

npy_dir  = '%s/proc/57519/avg_flags' %(work_dir)

basename = 'mjd57519_avg'
target = 2
...
if do_get_flags:
    get_and_save_avgdata_flags(basename, avg_file, tdur=1e6, target_id=target)

# The following reads and applies the flags
if do_apply_flags:
    tstart = time.time()
    tvals = np.load("%s/%s_times.npy" %(npy_dir, basename))
    flag_vals = np.load("%s/%s_flags.npy" %(npy_dir, basename))
    tmids, dt = apply_flags_from_avg(dat_file, tvals, flag_vals, tdur=3.0, no_write=no_write)
    dt = time.time() - tstart
