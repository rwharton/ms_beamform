#############################################
###  Hacky version just appending "old"   ###
###  to ms tasks to get the original      ###
###  behavior.  Fix this soon !!!         ###
#############################################
...
def average_visibilities(infile, ell_ems, beam_nums, spws=[0], tstep=1.0, nproc=4, 
    ...
    Tsec = np.diff(ms.rangeold('time')['time'])[0]
    Niter = int(Tsec / tstep) + 1
    #Niter = 60
    print(Niter, Tsec)

    # Default columns : array_id, field_id, data_desc_id, time
    ms.selectinitold(datadescid=0, reset=True)
    selection = {'field_id' : [target_id, phase_id], 'uvdist' : uvlim}
    ms.selectold(items = selection)
    ms.selectpolarizationold( pols )
    ...
    ms.iterinitold(["TIME"], tstep, adddefaultsortcolumns=False)
    ms.iteroriginold()
    ...
    for jj in range(Niter):
        ...
        rec = ms.getdataold(dcols, ifraxis=True)
...
def get_and_save_spw_freqs_save(infile, spw_list, basename=""):
    """
    Save freqs in MHz
    """
    for spw in spw_list:
...
def get_and_save_spw_freqs(infile, spw_list, basename=""):
    ...
    for spw in spw_list:
        ...
        #outfile = "%s_freqs_spw%02d.npy" %(basename, spw)
        #np.save(outfile, freqs / 1e6)
        spw_freqs_list.append(freqs)

    spw_freqs = np.hstack( spw_freqs_list )
    np.save("%s_freqs.npy" %basename, spw_freqs/1e6)
    return
...
def double_check_freqs(infile, spw_list, tstep=0.01, field=2):
    ...
    for spw in spw_list:    
        ...
        ms.open(infile)
        ms.selectinitold(datadescid=spw)
        ms.selectold({'field_id' : field})
        ms.iterinitold(["TIME"], tstep, adddefaultsortcolumns=False)
        ms.iteroriginold()
        data_col = 'corrected_data'
        rec = ms.getdataold([data_col, "axis_info"], ifraxis=True)
