...
if __name__ == "__main__":
    ...
    infile = '%s/57519_part1.ms' %datdir
    basename = 'mjd57519_part1'
    src_base = "MJD57519"
    ...
    beam_locs = beam_list[0:1000]
    beam_nums = beam_nums[0:1000]
    ...
    if do_get_mjd:
        tfiles = glob.glob("%s_tt_step*.npy" %(babename))
        tfiles.sort()
        tt = np.load(tfiles[0])
        tdict = se.get_time_info(tt[0])
