...
def fit_all_chans_med(dd):
    ...
    for ii in xrange(nchans):
        ...
        avgs.append( avg )
        sigs.append( sig )
        likes.append( lnlike(res['x'], x, y, yerr) )
...
def filter_FITS(infile, outfile):
    freqs, dd = get_data(infile)
    avgs, sigs, likes = fit_all_chans_med(dd)
...
def filter_many_FITS(indir, outdir, debug=False):
...
def multi_filter_FITS(nproc, indir, outdir, debug=False):
    infiles = glob.glob("%s/*fits" %indir)
    fnames = [ ff.split("/")[-1] for ff in infiles ]
    outfiles = [ "%s/%s" %(outdir, ff) for ff in fnames ]
    N = len(infiles)
    ...
    with closing(multiprocessing.Pool(processes=nproc)) as pool:
        results = [ pool.apply_async(filter_FITS,
                    args=(infiles[ii], outfiles[ii])) for ii in xrange(N)]
...
base_dir = "%s/part%d" %(top_dir, part)

print base_dir

indir  = "%s/npy_files" %base_dir
outdir = "%s/psrfits" %base_dir

filter_many_FITS(indir, outdir, debug=debug)
